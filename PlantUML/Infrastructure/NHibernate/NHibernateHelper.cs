...
public static class NHibernateHelper
{
    ...
    public static Configuration BuildConfiguration()
    {
        ...
        var cfgPath = Path.Combine(baseDir, "NHibernate", "NHibernate.cfg.xml");
        ...
        var mappingsDir = Path.Combine(baseDir, "NHibernate", "Mappings");
        if (Directory.Exists(mappingsDir))
        {
            foreach (var mappingFile in mappingsToLoad)
            {
                var filePath = Path.Combine(mappingsDir, mappingFile);
    ...
    var envConn = System.Environment.GetEnvironmentVariable("NH_CONNECTION");
        ...
        try
        {
            var dataDir = Path.Combine(Directory.GetCurrentDirectory(), "InitializeDb", "Data");
